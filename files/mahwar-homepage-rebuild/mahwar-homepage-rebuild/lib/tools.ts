...
export interface Tool {
  ...
  featured?: boolean;
  /**
   * Expected `?panel=` value on /dashboard. Values marked `null` were not
   * visible from the public route list — the integrating agent must look
   * them up in the project's panel registry (see INTEGRATION.md §5).
   */
  panel: string | null;
  href: string; // placeholder "#" until wired
}
