...
**Cinematic pass, anime.js v4 used properly.** `lib/anime.ts` is the anime side of the motion vocabulary (Framer keeps scroll-linked, pointer and layout motion; anime owns choreographed entrances and SVG line work). The hero is one `createTimeline()`: the axis draws, the headline rises, capital and goods traces (`components/ui/FlowTraces.tsx`, seeded and generative) draw in from both edges toward one axis, and the centre node morphs coin ↔ container with `svg.morphTo`. The Gulf map draws its six coastlines west → east with `svg.createDrawable`, then corridors by kind, then hubs and labels; flows and vehicles start only afterwards. The Gulf scene builds itself on first view (towers rise, cranes draw stroke by stroke, loads drop). `/login` runs the same timeline standard, with the boot log driven by an anime stagger and the new `AxisMark` line-mark. Every timeline honours `prefers-reduced-motion`.

**A privacy layer that exists in code, not in copy.** The saved session is sealed at rest with AES-GCM-256 using a non-extractable device key kept in IndexedDB (`lib/privacy/vault.ts`); persistent (localStorage) or ephemeral (sessionStorage, forgotten on tab close) is the user's choice. `PRIV` opens the privacy centre inside the terminal — a live inventory read from the browser, export, and a confirmed wipe that also signs out. `/privacy` was rewritten to describe exactly what the software does (the previous text claimed end-to-end encryption and SAMA alignment; neither was true). A one-time first-visit notice replaces consent theatre — there is no tracking to consent to. `/dashboard`, `/login` and `/api/session` are `no-store` and `noindex`; upstream news requests carry no referrer, cookies or visitor identity. Set `NEXT_PUBLIC_PRIVACY_CONTACT` before going live.

**Palette hygiene.** `lib/chartTheme.ts` is now the only file under `components/**` and `lib/**` with brand hex values. Twenty-three components that hard-coded colours — including the Tailwind default rainbow (`#3B82F6`, `#F59E0B`, `#8B5CF6`, `#EC4899`) in acquisition-cost, landed-cost, TCO, CCC, EOQ, S&OP, supplier-scorecard and facility-location charts — read `T.colors.*` / `T.series[i]` instead. The categorical palette no longer contains red. Legitimate literals remain: print-mode CSS in `BIReportEngine`, the Google "G" brand colours, and `themeColor` metadata.
