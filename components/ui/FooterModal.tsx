...
const FooterModal = ({ isOpen, onClose, type }: FooterModalProps) => {
  ...
  const renderContent = () => {
    ...
    if (key.includes("about") || key.includes("من نحن")) {
      return (
        <div className="space-y-6 text-fg-2">
          <div className="flex items-center gap-3 pb-4 border-b border-line">
            <div className="p-2.5 rounded-xl bg-emerald/10 text-emerald border border-emerald/30">
          ...
          <div className="space-y-4 text-xs leading-relaxed font-sans">
            ...
            <div className="p-4 bg-ink-3 border border-line rounded-xl space-y-2 mt-4">
              ...
              <div className="flex items-center gap-2 mt-3 pt-3 border-t border-line">
                <div className="w-8 h-8 rounded-full bg-ink-5 border border-line-strong flex items-center justify-center text-fg-3 font-bold font-serif">
                  M
                </div>
                <div>
                  <p className="text-fg-3 font-mono text-[10px] uppercase">
                    {isAr ? "تم التصميم والتطوير بواسطة" : "Architected & Developed By"}
                  </p>
                  <p className="text-fg font-bold">Muhammad Sarmad Nadeem</p>
                </div>
              </div>
            </div>
          </div>
    ...
    if (key.includes("contact") || key.includes("اتصل بنا")) {
      return (
        <div className="space-y-6 text-fg-2">
          ...
          <div className="space-y-4 text-xs leading-relaxed font-sans">
            <div className="p-5 bg-ink-3 border border-line rounded-xl space-y-5">
              <div>
                <h4 className="font-bold text-fg mb-1">{isAr ? "الشركة المطورة" : "Developer & Publisher"}</h4>
                <p className="text-fg-2 font-mono text-sm font-bold text-emerald">{isAr ? "زينر انك (Zener Inc.)" : "Zener Inc."}</p>
              </div>
              <div className="pt-4 border-t border-line">
                <h4 className="font-bold text-fg mb-2">{isAr ? "فريق الهندسة والتطوير" : "Lead Engineering & Support"}</h4>
                <p className="text-fg-2 font-bold">Muhammad Sarmad Nadeem</p>
                <div className="flex flex-col gap-1 mt-2">
                  <p className="text-fg-3 font-mono text-[11px]">Email: sarmadnadeem727@gmail.com</p>
                  <p className="text-fg-3 font-mono text-[11px]">GitHub: @sarmadnadeem727-cpu</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6 text-fg-2">
          ...
          <div className="space-y-4 text-xs leading-relaxed font-sans">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-4">
              <div className="p-4 bg-ink-3 border border-line rounded-xl space-y-2">
        ...
        <div className="space-y-6 text-fg-2">
          ...
          <div className="space-y-4 text-xs leading-relaxed font-sans">
            ...
            <div className="space-y-3">
              <div className="p-3.5 bg-ink-3 border border-line rounded-lg">
                <h4 className="font-bold text-fg mb-1 font-mono text-xs">
    ...
    if (key.includes("licens") || key.includes("تراخيص")) {
      return (
        <div className="space-y-6 text-fg-2 text-center py-2">
          <div className="w-14 h-14 bg-emerald/10 border border-emerald/30 rounded-2xl flex items-center justify-center mx-auto text-emerald">
          ...
          <div>
            ...
            <p className="text-xs text-fg-3 font-mono mt-1">
              {isAr ? "مسجلة ومحمية للمؤسسات والشركات الاستثمارية" : "Certified Workspace License & Intellectual Property Rights"}
            </p>
          </div>

          <p className="text-xs text-fg-2 leading-relaxed max-w-md mx-auto">
            {isAr
          ...
          <div className="p-4 bg-ink-3 border border-line rounded-xl font-mono text-xs max-w-sm mx-auto space-y-2">
            <div className="flex justify-between text-fg-3 text-[10px] uppercase tracking-wider font-bold">
              <span>{isAr ? "معرف الترخيص النشط" : "ACTIVE LICENSE ID"}</span>
              <span className="text-emerald">VERIFIED</span>
            </div>
            <div className="text-fg font-bold text-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              ...
              <div
                ...
                className="p-3 bg-ink-3 border border-line hover:border-emerald rounded-lg transition-all cursor-pointer group flex flex-col justify-between"
              >
                <div>
                  <span className="text-[9px] font-mono font-bold text-emerald tracking-wider uppercase">
                    {mod.tag}
                  </span>
                  <h4 className="text-xs font-bold text-fg group-hover:text-emerald transition-colors mt-0.5">
                    {mod.title}
                  </h4>
                </div>
                <div className="flex items-center gap-1 text-[10px] font-mono text-fg-3 group-hover:text-emerald pt-2 mt-2 border-t border-line">
    ...
    if (key.includes("indices") || key.includes("مؤشرات")) {
      return (
        <div className="space-y-6 text-fg-2">
          ...
          <div className="border border-line rounded-xl overflow-hidden bg-ink-2 shadow-2xs">
            <table className="w-full text-left font-mono text-xs">
              <thead>
                <tr className="bg-ink-3 text-fg-3 border-b border-line text-[10px] uppercase font-bold">
              ...
              <tbody className="divide-y divide-line">
                {[
                  ...
                  <tr key={i} className="hover:bg-ink-3 transition-colors">
                    <td className="p-3 text-fg font-bold">{idx.name}</td>
                    <td className="p-3 text-fg-3 text-[11px]">{idx.market}</td>
                    <td className="p-3 text-fg">{idx.val}</td>
                    <td className={`p-3 font-bold ${idx.up ? "text-emerald font-bold" : "text-neg"}`}>
                      {idx.chg}
                    </td>
                  </tr>
                ))}
    ...
    return (
      <div className="py-16 text-center text-fg-3 font-mono text-xs italic">
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-6 selection:bg-emerald selection:text-ink-0">
          {/* Backdrop */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          <motion.div
            ...
            className={`
              relative w-full max-w-2xl bg-ink-2 border border-line rounded-2xl shadow-2xl p-6 md:p-8 overflow-hidden z-10 font-sans
              ${isAr ? 'font-arabic' : ''}
            `}
            dir={isAr ? "rtl" : "ltr"}
          >
            ...
            <button
              onClick={onClose}
              className="absolute top-5 right-5 p-2 bg-ink-4 border border-line rounded-full text-fg-3 hover:text-fg hover:bg-ink-5 transition-colors z-20 cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
