    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 panel-input p-5">
        ...
        <button
          onClick={exportPDF}
          disabled={exporting}
          className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg bg-emerald hover:bg-emerald-light text-ink-0 font-mono font-bold text-xs uppercase tracking-wider transition-all cursor-pointer shadow-xs"
        >
          <Download size={14} />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 font-mono">
        ...
        <div className="p-4 bg-ink-2 rounded-xl border border-line shadow-xs">
          <span className="text-[10px] text-fg-3 uppercase font-bold block mb-1">Implied EV / EBITDA</span>
          <span className="text-2xl font-extrabold text-fg">{impliedEvEbitda}x</span>
          <span className="text-[10px] text-fg-3 block mt-1">Vs {headlineEvEbitda}x Headline</span>
        </div>

        <div className="p-4 bg-ink-2 rounded-xl border border-line shadow-xs">
          <span className="text-[10px] text-fg-3 uppercase font-bold block mb-1">Net Cash Outlay</span>
          <span className="text-2xl font-extrabold text-fg">SAR {netCashOutlay.toLocaleString()}M</span>
          <span className="text-[10px] text-fg-3 block mt-1">Cash Required at Closing</span>
        </div>

        <div className="p-4 bg-ink-2 rounded-xl border border-line shadow-xs">
          <span className="text-[10px] text-fg-3 uppercase font-bold block mb-1">Advisory & Legal Fees</span>
          <span className="text-2xl font-extrabold text-fg">SAR {advisoryFeesAmount}M</span>
          <span className="text-[10px] text-fg-3 block mt-1">{advisoryFeesPct}% of Purchase Price</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="panel-input p-5 space-y-4">
          ...
          <div className="space-y-3.5 text-xs font-sans">
            <div>
              <label className="text-fg-2 font-medium block mb-1">Headline Purchase Price (SAR M)</label>
              <input
                type="number"
                value={purchasePrice}
            ...
            <div>
              <label className="text-fg-2 font-medium block mb-1">Target Net Debt Assumed (SAR M)</label>
              <input
                type="number"
                value={assumedDebt}
            ...
            <div>
              <label className="text-fg-2 font-medium block mb-1">Advisory & Legal Fees (% of Price)</label>
              <input
                type="number"
                step="0.1"
            ...
            <div>
              <label className="text-fg-2 font-medium block mb-1">Integration & Restructuring (SAR M)</label>
              <input
                type="number"
            ...
            <div>
              <label className="text-fg-2 font-medium block mb-1">Net Working Capital Adjustment (SAR M)</label>
              <input
                type="number"
                value={workingCapAdj}
            ...
            <div>
              <label className="text-fg-2 font-medium block mb-1">Earn-outs & Deferred Consideration (SAR M)</label>
              <input
                type="number"
                value={earnOuts}
            ...
            <div>
              <label className="text-fg-2 font-medium block mb-1">Target LTM EBITDA (SAR M)</label>
              <input
                type="number"
                value={targetEbitda}
            ...
            <table className="w-full text-xs font-mono text-left rtl:text-right border-collapse">
              ...
              <tbody className="divide-y divide-line">
                {waterfallData.map((item, idx) => (
                  <tr key={idx} className="hover:bg-ink-3">
                    ...
                    <td className="py-2.5 px-3 text-right text-fg-3">
